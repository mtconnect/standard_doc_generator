...
\glspl{data entity} describe data that can be reported by a piece of equipment.  In the \gls{Device Information Model}, \glspl{data entity} are defined as \block{DataItem} elements (See \sect{DataItem Types} and \sect{DataItem SubTypes}).
...
Both \block{Component} and \block{Composition} elements are \gls{Lower Level} child elements representing the sub-parts of the parent \block{Component}.  However, there are distinct differences between \block{Component} and \block{Composition} type elements.
...
\input model-sections/RelationshipTypesforDataItem.tex
...
This section provides semantic information for \block{Reference} and its types.
...
\input model-sections/SolidModel.tex

\input model-sections/Specifications.tex

\input model-sections/LimitsforProcessSpecification.tex
