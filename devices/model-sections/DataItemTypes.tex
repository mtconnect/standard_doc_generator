...
Many types of data items provide two forms of data: a value (reported as either a \texttt{SAMPLE} or \texttt{EVENT}) and a health status (reported as a \texttt{CONDITION}).  These \block{DataItem} types \textbf{MAY} be defined in more than one \property{category} based on the data that they report.
...
The value of the \block{Wattage} used or generated by a component over an interval of time.
