...
\begin{itemize}
    ...
    \item \textbf{DEPRECATED} A \block{DataItem} with \property{representation}[DataItem] \texttt{DISCRETE} \textbf{MUST} have a \property{category}[DataItem] \texttt{EVENT}
...
\begin{itemize}
    \item Note: See \sect{Representation Schema Diagrams} for XML schema.
\end{itemize}
...
\texttt{DISCRETE} \textbf{MUST} only be used with a \block{DataItem} with a \property{category} of \texttt{EVENT}.
